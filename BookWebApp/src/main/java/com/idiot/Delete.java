...
public class Delete extends HttpServlet{
	...
	static String query="delete from BookList where id=?";
		@Override
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			try {
				...
				if (count>0) {
					out.println("<h2 class='bg-success text-light text-center'>Record is successfully Deleted</h2>");
					
				}else {
					out.println("<h2 class='bg-danger text-light text-center'>Record is not Deleted</h2>");
				}
			...
			out.println("<br>");
			out.println("<a href='showlist' <button class='btn btn-outline-secondary d-block'> Show List</button></a>");
			out.println("</div");
		}
		@Override
